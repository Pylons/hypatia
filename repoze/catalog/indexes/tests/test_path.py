...
class PathIndexTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._index = CatalogPathIndex('path')
        self._values = {
    ...
    def _populateIndex(self):
        for k, v in self._values.items():
            self._index.index_doc( k, v )

    def test_interfaces(self):
        from repoze.catalog.interfaces import ICatalogIndex
        from zope.interface.verify import verifyClass
        from repoze.catalog.indexes.path import CatalogPathIndex
        verifyClass(ICatalogIndex, CatalogPathIndex)

    def testEmpty(self):
        self.assertEqual(self._index.numObjects() ,0)
        self.assertEqual(self._index.getEntryForObject(1234), None)
        self._index.unindex_doc( 1234 ) # nothrow
        result = self._index.apply({"suxpath":"xxx"})
    ...
    def testRoot(self):
        ...
        for comp, level, results in tests:
            for path in [comp,"/"+comp,"/"+comp+"/"]:
                res = self._index.apply({"query":path,"level":level})
                lst = list(res.keys())
                self.assertEqual(lst,results)

        for comp,level,results in tests:
            for path in [comp,"/"+comp,"/"+comp+"/"]:
                res = self._index.apply({"query":((path,level), )})
                lst = list(res.keys())
                self.assertEqual(lst,results)
    ...
    def testComplexOrTests(self):
        ...
        for lst ,level,results in tests:
            ...
            res = self._index.apply({"query":lst,"level":level,"operator":"or"})
            lst = list(res.keys())
            self.assertEqual(lst,results)
    ...
    def testComplexANDTests(self):
        ...
        for lst ,level,results in tests:
            res = self._index.apply(
                {"query":lst,"level":level,"operator":"and"})
            lst = list(res.keys())
            self.assertEqual(lst,results)
